...
/**
 * <code>KeyboardLookHandler</code> defines an InputHandler that sets input to
 * be controlled similar to First Person Shooting games. By default the commands
 * are, WSAD moves the camera forward, backward and strafes. The arrow keys
 * rotate and tilt the camera.
 */
public class KeyboardWorldInputHandler extends InputHandler {

	private final KeyForwardAction forward;
	private final KeyBackwardAction backward;
	private final KeyStrafeLeftAction sLeft;
	private final KeyStrafeRightAction sRight;
	private final KeyRotateRightAction right;
	private final KeyRotateLeftAction left;
	private final KeyStrafeDownAction down;
	private final KeyStrafeUpAction up;

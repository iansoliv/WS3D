...
import static util.Constants.IMAGES_PATH;
...
public class Cage extends Container {
    ...
    public Cage(double x, double y, Environment ev, MaterialState ms, TextureState ts) {
        ...
        try {
            ...
            y2 = y;
            this.ts = ts;
            sf = new Thing.ThingShapeFactory(IMAGES_PATH +"/empty_cage_p.3DS", this);
            shape = sf.getNode(0.03f);
            shape.setRenderState(ms);
            setTexture(IMAGES_PATH +"/texture9.jpeg");
    ...
    public void setStatus(int cageStatus) {
        ...
        switch (cageStatus) {
            case Constants.FULL_OPENED_APPLE:
                updateShape(IMAGES_PATH +"/apple_cage_open_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_CLOSED_APPLE:
                updateShape(IMAGES_PATH +"/apple_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_NUT:
                updateShape(IMAGES_PATH +"/nut_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_NUT:
                updateShape(IMAGES_PATH +"/nut_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.EMPTY_OPENED:
                updateShape(IMAGES_PATH +"/empty_cage_p.3DS", 0.03f, this.e);
                break;
            case Constants.EMPTY_CLOSED:
                updateShape(IMAGES_PATH +"/empty_cage_closed_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_JEWEL:
                updateShape(IMAGES_PATH +"/jewel_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_JEWEL:
                updateShape(IMAGES_PATH +"/jewel_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_MNJ:
                updateShape(IMAGES_PATH +"/mnj_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_MNJ:
                updateShape(IMAGES_PATH +"/mnj_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_MN:
                updateShape(IMAGES_PATH +"/mn_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_MN:
                updateShape(IMAGES_PATH +"/mn_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_MJ:
                updateShape(IMAGES_PATH +"/mj_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_MJ:
                updateShape(IMAGES_PATH +"/mj_cage_p.3DS", 0.03f, this.e);
                break;

            case Constants.FULL_OPENED_NJ:
                updateShape(IMAGES_PATH +"/nj_cage_open_p.3DS", 0.03f, this.e);
                break;
            case Constants.FULL_CLOSED_NJ:
                updateShape(IMAGES_PATH +"/nj_cage_p.3DS", 0.03f, this.e);
                break;
            default:
